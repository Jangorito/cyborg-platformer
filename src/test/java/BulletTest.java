...
class BulletTest {
    ...
    void setUp() {
        ...
        // static globals in canvas
        canvas.activeBullets = new ArrayList<>();
        canvas.enemies = new ArrayList<>();

        // We will not create a new bullet instance to be shared by each test because different field
    ...
    void travelledDistanceIsZeroAtStart() {
        bullet b = new bullet(100, 200);                            // create new bullet image
        b.startPoint = new Point(100, 200);                         // set bullet start point
    ...
    void travelledDistanceComputesCorrectly() {
        bullet b = new bullet(0, 0);                                // create new bullet image
        b.startPoint = new Point(0, 0);                             // set bullet start point

        b.x = 3;                                                          // set new bullet x position
        b.y = 4;                                                          // set new bullet y position

        assertEquals(5.0, b.travelledDistance(), 0.0001);   // distance travelled   :   should be 5
    }

    @Test
    @DisplayName("update() moves bullet by speed when no collisions and under 600px")
    void updateMovesBulletWhenClear() {
        bullet b = new bullet(0, 0);                                // create new bullet image
        b.startPoint = new Point(0, 0);                             // set bullet start point
        b.speed = 10;                                                     // set bullet speed
        canvas.activeBullets.add(b);                                      // add bullet to canvas' activeBullets list

        b.update();                                                       // update bullet

        assertEquals(10, b.x);                                   // x value                 : should be 10
        assertTrue(canvas.activeBullets.contains(b),
                "Bullet should remain active before 600px");     // list contains bullet?   : should be True
    }

    @Test
    @DisplayName("update() removes bullet after travelling 600px or more")
    void updateRemovesBulletAfter600px() {
        bullet b = new bullet(600, 0);                              // create new bullet image
        b.startPoint = new Point(0, 0);                             // set bullet start point
        b.speed = 10;                                                     // set bullet speed
        canvas.activeBullets.add(b);                                      // add bullet to canvas' activeBullets list
